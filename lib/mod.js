...
const Mod = module.exports.Mod = function (modjule) {
  ...
  self.wrapStdAws = function (arg0, ...args) {
    ...
    let   [old, awsModName, obj, ...names] = [arg0, ...args];           // <-- signature
  ...
  function wrapAws(awsModName, obj, name, errOk =false) {
    return function (params, callback) {
      ...
      // ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      function logit(err, data, rest) {
        console.log(`AWS:${awsModName}.${name} done:`, {params}, cleanResult({err, data, rest}));

        // Specific logging for specific fns
        if (awsModName === 's3' && name === 'putObject') {
          const {Bucket, Key, Body} = params;
          console.log(`Saved ${Body.length} bytes to s3://${Bucket}/${Key}`);
        }
      }
    };
  }
